...
const EnterCodePasswd = (props) => {
    ...
    try {
      await askForCode(email).then((resData) => {
        if (resData.createVerficationCode) {
          setMessage('Código enviado. Tiene una validez de 3 minutos');
        }
      });
    } catch (err) {
  ...
  return (
    <ImageBackground
      source={require('../../../../assets/images/bck14_1.jpg')}
      ...
      <View style={styles.opaque}>
        ...
        />
        <EnterCodeInput email={email} setError={setError} theme={theme} />
        <View style={{flex: 0.2}} />
      </View>
...
export default EnterCodePasswd;
